 ...
 * 
 * Catatan      :: Custom Button Dengan Dropdown Menu
 * 
 */
...
namespace GoogleCloudStorage.Components {
    ...
    public sealed class SplitButton : Button {
        ...
        public override ContextMenuStrip ContextMenuStrip {
            get => this.SplitMenuStrip;
            set => this.SplitMenuStrip = value;
        }

        [DefaultValue(null)]
        public ContextMenu SplitMenu {
            get => this.m_SplitMenu;
            set {
        ...
        private void PaintTextandImage(Graphics g, Rectangle bounds) {
            ...
            this.CalculateButtonTextAndImageLayout(ref bounds, out Rectangle text_rectangle, out Rectangle image_rectangle);
        ...
        private void CalculateButtonTextAndImageLayout(ref Rectangle content_rect, out Rectangle textRectangle, out Rectangle imageRectangle) {
            ...
            switch (this.TextImageRelation) {
                case TextImageRelation.Overlay:
                    ...
                    textRectangle = OverlayObjectRect(ref content_rect, ref text_size, this.TextAlign); // Rectangle.Inflate(content_rect, -4, -4);
        ...
        private void LayoutTextBeforeOrAfterImage(Rectangle totalArea, bool textFirst, Size textSize, Size imageSize, out Rectangle textRect, out Rectangle imageRect) {
            ...
            if (textFirst) {
                final_text_rect = new Rectangle(totalArea.Left + offset, AlignInRectangle(totalArea, textSize, this.TextAlign).Top, textSize.Width, textSize.Height);
                final_image_rect = new Rectangle(final_text_rect.Right + element_spacing, AlignInRectangle(totalArea, imageSize, this.ImageAlign).Top, imageSize.Width, imageSize.Height);
            }
            else {
                final_image_rect = new Rectangle(totalArea.Left + offset, AlignInRectangle(totalArea, imageSize, this.ImageAlign).Top, imageSize.Width, imageSize.Height);
                final_text_rect = new Rectangle(final_image_rect.Right + element_spacing, AlignInRectangle(totalArea, textSize, this.TextAlign).Top, textSize.Width, textSize.Height);
            }
        ...
        private void LayoutTextAboveOrBelowImage(Rectangle totalArea, bool textFirst, Size textSize, Size imageSize, out Rectangle textRect, out Rectangle imageRect) {
            ...
            if (textFirst) {
                final_text_rect = new Rectangle(AlignInRectangle(totalArea, textSize, this.TextAlign).Left, totalArea.Top + offset, textSize.Width, textSize.Height);
                final_image_rect = new Rectangle(AlignInRectangle(totalArea, imageSize, this.ImageAlign).Left, final_text_rect.Bottom + element_spacing, imageSize.Width, imageSize.Height);
            }
            else {
                final_image_rect = new Rectangle(AlignInRectangle(totalArea, imageSize, this.ImageAlign).Left, totalArea.Top + offset, imageSize.Width, imageSize.Height);
                final_text_rect = new Rectangle(AlignInRectangle(totalArea, textSize, this.TextAlign).Left, final_image_rect.Bottom + element_spacing, textSize.Width, textSize.Height);
